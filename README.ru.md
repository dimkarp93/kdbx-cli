...
Некоторые программы читают stdin, только когда терминала нет: например, `psql -W` при наличии `/dev/tty` спрашивает пароль там, поэтому в интерактивном шелле секрет из stdin он игнорирует (из cron или CI это работает). Для `psql` используйте `PGPASSWORD` через `secrets` или `PGPASSFILE` через `files`.
...
Файл создаётся с правами `0600`, поэтому его принимают программы, которые отвергают файлы с учётными данными, доступные другим (`mysql --defaults-extra-file`, `PGPASSFILE` в libpq):

```sh
kdbx-cli --secret-file=pgpass -- sh -c 'PGPASSFILE="$1" exec psql -h db -U app' sh '{{pgpass}}'
```

`ssh` и `ssh-agent` при запуске закрывают все унаследованные дескрипторы выше stderr, поэтому `ssh -i '{{ssh-key}}'` работать не может. Вместо этого загрузите ключ в агент из шелла-обёртки (в `-i` передаётся публичный ключ, чтобы выбрать identity):
...
Хелпер отдаёт один и тот же секрет на любой запрос. Для `git` по HTTP указывайте имя пользователя в URL (`https://me@host/repo.git`), чтобы git спрашивал только пароль. Без него секрет уйдёт ещё и как имя пользователя, а это работает только на серверах, которые принимают токен в качестве имени (GitHub, Gitea).
...
Интеграционные тесты (реальные программы и сервисы в Docker, все каналы доставки, инварианты безопасности, записи терминала) живут в отдельном репозитории `kdbx-cli-tests` и гоняются против выпущенных версий `kdbx-cli`.
